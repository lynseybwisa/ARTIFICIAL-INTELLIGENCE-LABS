...
node_col = ['limegreen' if not node in route_list else 'coral' for node in G.nodes()]
peru_colored_edges = list(zip(route_list,route_list[1:]))
#color the edges as well
#print(peru_colored_edges)
edge_col = ['limegreen' if not edge in peru_colored_edges else 'coral' for edge in G.edges()]
...
nx.draw_networkx_edge_labels(G, node_pos, edge_labels={('SportsComplex','Siwaka'):'UnkRoad450m',
   ...
   ('ParkingLot','Mada'):'LangataRd700m'},font_color='teal',font_size='x-small')
plt.axis('off')
plt.show()